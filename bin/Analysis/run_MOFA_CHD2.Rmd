...
```{r}
select_samples <- phenotype_data[phenotype_data$phenotype_CHD2_CRISPR %in% c('Healthy', 'Diseased: CHD2'),]
...
outdir <- params$outdir
outfile = file.path(outdir, "model_CHD2.hdf5")
...
ggplot(df_wider, aes(x= Factor1,y=Factor2, color =color_by)) + 
  geom_point() +
...
lapply(assay_names, function(a) {
  ...
  f <- gsub("[| ,]", "_", a)
  outdur_disease <- file.path(outdir, 'CHD2')
